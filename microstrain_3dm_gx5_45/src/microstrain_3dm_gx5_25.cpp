...
namespace Microstrain
{
  ...
  void Microstrain::run()
  {
    ...
    //決セージのセットアップはうまく設定できなかったので削除。WINDOWSから設定したほうが確実
      ...
      case MIP_INTERFACE_CALLBACK_VALID_PACKET:
	    {
          ...
          switch(field_header->descriptor)
             ...
             case MIP_FILTER_DATA_ATT_QUATERNION:
             {
              ...
              // std::cout<<"euler:"<<curr_filter_angles_.roll<<" , "<<curr_filter_angles_.pitch<<" , "<<curr_filter_angles_.yaw<<std::endl;
            ...
            case MIP_FILTER_DATA_LINEAR_ACCELERATION:
            {
              ...
              imu_filtered_msg_.linear_acceleration.x = curr_filter_linear_acc_.x;
              imu_filtered_msg_.linear_acceleration.y = curr_filter_linear_acc_.y;
              imu_filtered_msg_.linear_acceleration.z = curr_filter_linear_acc_.z;
