...
//
//fprintf(o_fp,"%lf, %lf, %lf, %lf, %lf, %lf, %lf\n",
// (double)position.x, (double)position.y, (double)position.th, (double)g_drad_now,
// (double)o_x, (double)o_y, (double)o_rad);

//fprintf(o_fp,"%d, %d, %d, %d, %d\n", 1, 1, 1, 1, 1) ;
//fprintf(o_fp,"%lf, %lf, %lf, %lf, %lf\n", (double)position.x, (double)position.y, (double)position.th, (double)g_drad_now, (double)o_rad);
//printf("%f, %f, %f, %f, %f, %f, %f,\n", position.x, position.y, position.th, g_drad_now, o_x, o_y, o_rad);
	...
	for (num = 0; num < joy->axes.size(); num++){
		if (joy->axes[num] != 0){
			...
			case 5://5
				...
				if (joy->axes[num] > 0){
					//puts("上");
					if (wheel.v < OKATECH_LIMIT_SPEED_V - 0.01) {//0.05
						wheel.v += 0.01;//0.02
						runV_cv_slow();
					}
				}else{
...
void U_OKATECH::cmdVelReceived(const geometry_msgs::Twist::ConstPtr& twist){
	...
	double cmd_v,cmd_dv;
	double cmd_v_ago  = 0.0 ,cmd_dv_ago = 0.0;
	cmd_v  = twist->linear.x;
	cmd_dv = twist->angular.z * OKATECH_TREAD * 0.5;

	if(joy_use_flag || stop_flag && cmd_v > 0.0)return;
	if((double)(clock() - start_time)/CLOCKS_PER_SEC < _cmd_control_period * 0.001 ){
		return;
	}else{
		start_time = clock();
	}
	...
	if(fabs(cmd_v_ago - cmd_v) > _cmd_acc_limit)
		cmd_v = (cmd_v_ago > cmd_v ? (cmd_v_ago - _cmd_acc_limit) : (cmd_v_ago + _cmd_acc_limit));
	if (fabs(cmd_dv_ago - cmd_dv) > _cmd_ang_acc_limit)
		cmd_dv = (cmd_dv_ago > cmd_dv ? (cmd_dv_ago - _cmd_ang_acc_limit) : (cmd_dv_ago + _cmd_ang_acc_limit));
	////////////////////////ここまで//////////////////////////////////////////
	runV_L( cmd_v - cmd_dv);
	runV_R( cmd_v + cmd_dv);
}
...
//	o_fp = fopen("/home/robo/smpl2_0903.csv","w");
