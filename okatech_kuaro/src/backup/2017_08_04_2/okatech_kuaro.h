...
private:
	...
	int read_data1_, read_data2_, read_data3_, read_data4_;

	static boost::timer::auto_cpu_timer time_interval;

protected:
	void notify(const std::string& str);//TinyPowerからの受信
};

class OKATECH
{
private:
	std::string message_ToTinyPower_;
	serial::SerialPort TinPow;
	Observer observer;
	void sendMVV(void);
	boost::thread sendMVV_thread_;
...
public:
  ...
  ros::Subscriber sub_velcmd_;
	...
	nav_msgs::Odometry odom_publish();
