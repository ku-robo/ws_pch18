...
OKATECH::OKATECH()
{
  ...
  pub_ToSerial_= n.advertise<std_msgs::String>("okatech_string",10);
  ...
  while (ros::ok())
  {
    if (TinPow.open(Port))
    {
      std::cout<<"OKATECH 通信開始"<<std::endl;
      break;
    }
    else
    {
  ...
  TinPow.attach(&observer);
  sendMVV_thread_ = boost::thread(&OKATECH::sendMVV, this);
}


OKATECH::~OKATECH()
{
  TinPow.detach(&observer);
  TinPow.close();

  if (sendMVV_thread_.joinable())
  {
    sendMVV_thread_.join();
  }
}
...
void Observer::notify(const std::string& str)
{
  ...
  else
  {
		...
		sscanf(extraction.c_str(), "%d,%d,%d,%d", &read_data1_, &read_data2_, &read_data3_, &read_data4_);
...
void OKATECH::SendComandToSerial_Callback(const geometry_msgs::Twist::ConstPtr& vel_msg){
  ...
  message_ToTinyPower_= ToStringCmd_linear_vel(vel_msg->linear.x);
  //pub_ToSerial_.publish(message_ToTinyPower_);
  TinPow.send(message_ToTinyPower_);
  if(vel_msg->linear.x>=0.0)
  {
    message_ToTinyPower_= ToStringCmd_angular_vel(vel_msg->angular.z);
  }
  else
  {
    message_ToTinyPower_= ToStringCmd_angular_vel(-vel_msg->angular.z);
